...
std::string Console::input(int x, int y, bool isPlus, bool ispassword, int mn)
{
    ...
    while (true)
    {
        ...
        if (isPlus && ch == 43)
        { // neu dang nhap thi co tao tai khoan moi
            return "+";
        }
...
void Console::change_information(bool manager)
{
    ...
    while (true)
    {
        ...
        case '5':
        {
            ...
            if (gender != "")
                cur.set_gender(bool(gender[0] - '0'));
            break;
        }
        ...
        case '6':
        {
            ...
            if (phone_number != "")
                cur.set_phone_number(phone_number);
            break;
        }
...
void Console::system_transaction_history()
{
    ...
    while (getline(infile, sentence))
    {
        ...
        while (i < sentence.size() && sentence[i] != ' ')
            transaction_time += sentence[i++];
        while (i < sentence.size())
            transaction += sentence[++i];
    ...
    { // Login
        ...
        else
        {
            ...
            if (cur.get_account().is_auto_password(cur.get_full_name(), cur.get_age(), cur.get_gender()))
            {
                while (true)
                {
                    if (Console::create_password() == 1)
                    { // bam tab hoac dang ki thanh cong
                        break;
                    }
                }
                continue; // tro ve giao dien dang nhap
